...
public class DemoAutoNewsCrawler extends BreadthCrawler {
    ...
    public DemoAutoNewsCrawler(String crawlPath, boolean autoParse) {
        ...
        for (int pageIndex = 0; pageIndex <= 100; pageIndex++) {
        	for (int pageIndex1 = 10000; pageIndex <= 99999; pageIndex++) {
            String seedUrl = String.format("http://www.xbiquge.la/%d/%d/", pageIndex,pageIndex1);
         ...
         * */
        this.addRegex("http://www.xbiquge.la/[0-9]{4}-[0-9]{2}-[0-9]{2}-[^/]+/");
        /**
    ...
    public void visit(Page page, CrawlDatums next) {
         ...
         */
        if (page.matchUrl("http://www.xbiquge.la/.*")) {
 
            /**
             * 通过 选择器 获取页面 标题以及 正文内容
             * */
            String title = page.select("li[class=lh-condensed]").first().text();
